			...
			<form type="get" id="filter-form">
				<table class="table table-striped table-filter" id="data-table-export1" >
					...
					<tr>
						...
						<th colspan="5">Overall Wambi Rating</th>
						<th>Total</th>
						...
						@foreach ($surveys as $element)
							<tr>
								...
								<td>
									...
									{{ $element->job_name() }}
									@elseif(array_has($job_titles,$element->job))
									{{$job_titles[$element->job]->value}}
									@endif
								</td>
								@for($i = 0; $i < $surveys_count; $i++)
									{{--*/ $q = 'q'.($i+1) /*--}}
